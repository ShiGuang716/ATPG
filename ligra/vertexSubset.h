...
struct vertexSubsetData {
  ...
  void toDense() {
    if (d == NULL) {
      printf("Convert to dense but keep sparse representation if it exists.\n");
      d = newA(D, n);
...
struct vertexSubsetData<pbbs::empty> {
  ...
  : n(_n), m(1), d(NULL), isDense(0) {
    #ifdef PRINT
    printf("A vertexSubset with a single vertex.\n");
    #endif
    s = newA(uintE, 1);
    s[0] = v;
  }
 ...
 vertexSubsetData<pbbs::empty>(long _n, tuple<bool, pbbs::empty>* _d)
  ...
  void toDense() {
    if (d == NULL) {
      #ifdef PRINT
      printf("Converts to dense but keeps sparse representation if it exists.\n");
      #endif
      d = newA(bool,n);
